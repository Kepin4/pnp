...
<script>
    ...
    function loadWithdrawRequests() {
        ...
        $.ajax({
            url: baseUrl + '/CTrans/ajaxWithdrawRequests',
            type: 'GET',
            data: formData,
            dataType: 'json',
        ...
        if (!(userLevel >= 1 && userLevel <= 3)) {
            if (!isProcessed) {
                actions += `<a href="${baseUrl}/CTrans/ProcessReqWithdraw/${item.kodereq}">
                            ...
                            <a href="${baseUrl}/CTrans/RefuseReqWithdraw/${item.kodereq}">
    ...
    function exportData(type) {
        const formData = $('#frmFilter').serialize();
        const url = `${baseUrl}/CExport/WithdrawRequest/${type}?${formData}`;
