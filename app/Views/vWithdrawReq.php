        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <h6 class="font-weight-bolder mb-0">Withdraw Request List</h6>
                </nav>
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        <div class="card-header pt-3 pb-1">
                            <form id="frmFilter" action="<?= base_url('/CTrans/WithdrawRequest') ?>" method="POST">
                                <?= csrf_field() ?>
                            ...
                            <div class="row">
                                <div class="col-7">
                                    <h4 class="pt-4">Withdraw Request List</h4>
                                ...
                                <?php if (!(session('level') >= 1 && session('level') <= 3)) { ?>
                                    <div class="col-5 text-end pt-4">
                                        <a href="<?= base_url('/CBase/NewWithdraw') ?>">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($dtReqTopup as $q) { ?>
                                            <tr class="text-xs">
                                                ...
                                                <td>
                                                    <?php if (!(session('level') >= 1 && session('level') >= 3)) { ?>
                                                        <a href=" <?= base_url('../CTrans/ProcessReqWithdraw/' . '/' . $q->kodereq) ?>">
                                                            <button class="bg-transparent border-0 text-xs" <?= (($q->status == 5 || $q->status == 8)  ? "disabled" : "") ?>>
                                                                Withdraw
                                                            </button>
                                                        </a>
                                                        |
                                                        <a href=" <?= base_url('../CTrans/RefuseReqWithdraw/' . '/' . $q->kodereq) ?>">
                                                            <button class="bg-transparent border-0 text-xs" <?= (($q->status == 5 || $q->status == 8)  ? "disabled" : "") ?>>
                                                                Refuse
                                                            </button>
                                                        </a>
...
<script>
    $(document).ready(function() {
        ...
        $('#ExportPDF').on('click', function() {
            SendReport(xForm, 'CExport/WithdrawRequest/PDF')
        });
        $('#ExportExcel').on('click', function() {
            SendReport(xForm, 'CExport/WithdrawRequest/Excel')
        });
    })
</script>